...
/*   Updated: 2021/10/06 18:20:10 by mminkjan      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */

# include "../includes/scop.h"

static void		copyfloats(GLfloat *dst, GLfloat *src, size_t len)
{
	for (int i = 0; i < len; i++)
	{
		dst[i] = src[i];
...
void			obj_parcer(t_cop *scop, t_buffer_data *buffer, char *str)
{
	...
	for (i = 0; i < VDUB && buffer->points[i] != '\0'; i++);
		
	scop->obj.vertices = (GLfloat*)malloc(sizeof(GLfloat) * i);
	copyfloats(scop->obj.vertices, buffer->points, i);
